...
def show_help_window(title, file_name):
    ...
    try:
        ...
        if not os.path.exists(path):
            messagebox.showerror("Ошибка", f"Файл справки не найден:\n{file_name}")
            return
...
def populate_main_window(dependencies, root, settings, main_frame, widgets, tvars):
    ...
    help_btn = ctk.CTkButton(header_frame, text="Help", width=50, height=30, 
                             fg_color="transparent", border_width=1, 
                             command=lambda: show_help_window("Главное окно", "Main_Window_Help.txt"))
    help_btn.pack(side="left", padx=5)
    ToolTip(help_btn, "Открыть справку (Блокнот)")
    ...
    widgets["generate_btn"] = ctk.CTkButton(top_gen_row, text="Генерировать", command=dependencies.generate_action, height=40, width=130)
    widgets["generate_btn"].pack(side="left", fill="x", expand=True)
    
    widgets["stop_btn"] = ctk.CTkButton(btns_frame, text="Отмена генерации", command=dependencies.stop_generation, state="disabled", fg_color="#ff5555", hover_color="#d63c3c", width=130, height=40)
    ...
    ai_model_label = ctk.CTkLabel(
        ai_indicator_frame, 
        text=f"⚡ {settings.get('OLLAMA_MODEL', 'Не выбрано')}", 
        font=("Roboto", 11),
    ...
    ToolTip(ai_model_label, "Нажмите для настройки AI")
    ...
    widgets["add_btn"] = ctk.CTkButton(add_to_anki_frame, text="✅ В Anki", command=dependencies.on_yes_action, width=100, fg_color="#2CC985", hover_color="#26AD72")
